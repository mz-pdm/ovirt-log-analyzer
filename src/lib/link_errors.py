...
def create_error_graph(log_freq, sender_freq, event_freq, message_freq,
    ...
    for t in sorted(d.keys(), key=lambda k: int(k)):
        for log in sorted(d[t].keys()):
            if sender_freq[d[t][log]['sender']] <= \
                  np.median(list(sender_freq.values())):
                s = ax1.scatter(int(t) // 1000 -
                                ...
                                marker='*',
                                zorder=3)
            if message_freq[d[t][log]['message']] <= np.median(
                  list(message_freq.values())):
                m = ax1.scatter(int(t) // 1000 -
                                ...
                                marker='o',
                                zorder=2)
            if event_freq[d[t][log]['event']] <= np.median(
                  list(event_freq.values())):
    ...
    for t in sorted(d.keys(), key=lambda k: int(k)):
        for log in sorted(d[t].keys()):
            if sender_freq[d[t][log]['sender']] < \
                  np.median(list(sender_freq.values())):
                G.add_node(datetime.utcfromtimestamp(int(t) /
                           1000).strftime("%H-%M-%S.%f\n%d.%m.%Y") +
    ...
    for t_susp, attr in sorted(G_copy.nodes(data=True),
                               ...
                               1000)):
        # check if mean number of following errors is greater
        # than previos
        # (look over 5 seconds)
        ...
        count_err_next = np.mean(timeline[int(t_su) // 1000 -
                                 min(map(int, d.keys())) // 1000:]) if \
                                 (int(t_su) // 1000 -
                                 min(map(int, d.keys())) // 1000 +
                                 step + 1 >= len(timeline)) \
                                 else
                                 np.mean(timeline[1 + int(t_su) //
                                         1000 - min(map(int, d.keys())) //
                                         1000:int(t_su) // 1000 -
                                         min(map(int, d.keys())) // 1000 +
                                         step + 1])
        count_err_prev = np.mean(timeline[0:1 + int(t_su) // 1000 -
                                    min(map(int, d.keys())) // 1000]) \
                        if (int(t_su) // 1000 - min(map(int, d.keys())) //
                        1000 - step <= 0) \
                        else np.mean(timeline[int(t_su) // 1000 -
                                     min(map(int, d.keys())) // 1000 -
                                     step:int(t_su) // 1000 -
                                     min(map(int, d.keys())) // 1000])
        next_point = max(map(int, d.keys())) if (int(t_su) // 1000 -
                         min(map(int, d.keys())) // 1000 + step + 1 >= \
                         len(timeline)) else int(t_su) // 1000 +
                         step + 1
        added_messages = []
        added_events = []
        prev_error = None
        ...
        if count_err_next > count_err_prev:
            for t_err in sorted(d.keys(), key=lambda k: int(k)):
                for log in sorted(d[t_err].keys()):
                    if (int(t_su) // 1000 < int(t_err) //
                          1000 < next_point):
                        if (prev_error is not None):
                            err_to_link = prev_error
                            link_label = link_message
                        else:
                            err_to_link = datetime.utcfromtimestamp(
                                          int(t_su) / 1000).strftime(
                                          "%H-%M-%S.%f\n%d.%m.%Y") +\
                                          '\nSender=(' + attr['sender'] +\
                                          ')\nEvent=[' + attr['event'] +\
                        ...
                        G.add_node(datetime.utcfromtimestamp(
                        ...
                        G.add_edge(err_to_link,
                                   datetime.utcfromtimestamp(
                                    ...
                                    ']\n' + 'Suspicious because of time',
                                    label=link_label)
                        prev_error = datetime.utcfromtimestamp(
                                        int(t_err)/ 1000).strftime(
                                        "%H-%M-%S.%f\n%d.%m.%Y") +\
                                      '\nSender=(' +\
                                      d[t_err][log]['sender'] +\
