            ...
            (let* ((beg (match-beginning 0))
              ...
              (dolist (tag tag-list)
                (cond
                 ((string-match "^Task/\\([1-9]\\)$" tag)
                  (ovirt-log-analyzer-add-text (- tag-beg 1) (make-string (string-to-number (match-string 1 tag)) ?>)
              ...
              (when visible-tags
                ...
                (while (re-search-forward "\\(Host\\|VM\\)=\\([^; |]*\\)" tag-end t)
                  (let ((beg (match-beginning 0))
                        (end (match-end 0))
                        (entity (match-string 1))
                        (value (match-string 2))
                        (properties (list 'invisible 'no
                                          'face 'font-lock-variable-name-face)))
                    (cond
                     ((string= entity "Host")
                      (setq properties (plist-put properties 'ovirt-log-analyzer-host value)))
                     ((string= entity "VM")
                      (setq properties (plist-put properties 'ovirt-log-analyzer-vm value))))
                    (add-text-properties beg end properties)
                    (ovirt-log-analyzer-add-text beg " ")
                    (cl-incf end)
                    (cl-incf tag-end)
                    (goto-char end))))
              (put-text-property tag-beg (1+ tag-end) 'invisible 'yes))
